...
| 项 | 卡在哪 | 怎么做 |
|---|---|---|
| **CI 启用** | 推送 token 缺 `workflow` scope，GitHub 拒绝写入 `.github/workflows/**` | 跑 `python tools/enable_ci.py`（引导授权后自动就位并推送） |
| **HTTPS 生效** | 腾讯云安全组未放行 443（TCP 握手由云网关代答，故"可连接"是假象） | 控制台放行 TCP 443，然后跑 `python tools/check_https.py` 验证 |

服务器侧 HTTPS 已全部就绪：证书有效至 2028-12-17，本机 TLSv1.3 握手 0.012s；
客户端已实现 HTTPS 优先 + 自动回退，放行后重启即自动加密，无需改任何配置。
