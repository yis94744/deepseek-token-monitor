...
- **DSH Harness 同步**：自动读取 DeepSeek Harness / DSH Desktop 的会话用量投影缓存（数据目录识别：`config.dsh.home` > `DSH_HOME` > `~/.dsh`；自动识别新版分片目录 `storages/session_projcache/sessions/*.json` 与旧版单文件 `session_projcache.json`），DSH 对话自动统计
...
- **实时计费**：按 DeepSeek 官网峰谷价计算费用（工作日高峰 09:00-12:00 与 14:00-18:00 价格翻倍，周末全天低谷价；2026-09-10 12:00 起 Flash 系列降价为 0.02/1/4 元每百万，高峰翻倍；2026-08-17 之前的调用按旧平峰价结算），单价可在配置中调整
- **分类统计**：按模型分别统计，支持 `deepseek-v4-flash` / `deepseek-v4-pro` / `deepseek-v4-flash-vision-exp` / `deepseek-v4.1-flash`（V4 Pro 于 2026-09-14 12:00 下线后自动按 V4.1 Flash 计费）
...
- **时段统计**：高峰/非高峰分时统计——不同时段按各自计费规则（工作日高峰 09:00-12:00 与 14:00-18:00 价格翻倍，周末全天低谷价），近 30 天堆叠柱状图 + 每日高峰/非高峰明细表（请求/Token/费用）
...
- **桌宠形态**：设置页可在「悬浮窗 / 桌宠」两种桌面挂件形态间切换。桌宠为透明底本体（默认水豚，可替换 `assets/pet.png`），可拖动、按压回弹；**点击桌宠弹出信息面板**，展示①当前余额 ②今日总消耗 ③今日当前 Key 消耗金额+Token 数量 ④当前计价时段（高峰显示「梁文峰」、低谷显示「梁文谷」）
...
**方式四：DSH Harness / DSH Desktop（自动，无需任何配置）**

1. 启动软件即可，DSH Desktop 会把各会话用量投影写到 `~/.dsh/storages/session_projcache/`（新版分片目录；老版本为单文件 `session_projcache.json`，自动兼容）
2. 软件每 5 秒只读同步，按会话差分导入 token 用量并计费（已导入过的会话只补增量，重启不重复计费）
3. 模型归属：手动映射 > 本地 API 解析 > `~/.dsh/settings.yaml` 默认模型（`agent-default-model`），解析不到时按 `unknown_model_fallback` 计费
...
- `dsh`：DSH Harness 同步设置（默认开启；数据目录定位 `home` 留空时按 `DSH_HOME` > `~/.dsh`，`projcache_path` 留空时自动探测新版分片目录或旧版单文件）
  - `home`：可选手动指定 DSH 数据目录（默认 `~/.dsh`）
  - `api_base` / `model_refresh_seconds` / `models`：DSH 本地 API 地址与模型解析缓存设置
...
- 五个数据源（本地代理 / CC Switch / DSH Harness / CodeBuddy / WorkBuddy）可在设置页「数据源开关」中分别开关，修改即时生效
